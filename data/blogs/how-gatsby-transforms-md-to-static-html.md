---
title: Gatsby で markdown ファイルが HTML に変換される仕組みを追ってみた
description: 気になったのでコードを読んでみて自分なりにまとめました
date: 2021-03-07 23:17
slug: how-gatsby-transformer-remark-works
tags: [Gatsby, remark]
---

本ブログでは markdown ファイルを記事データソースとして扱っています。このやり方は[公式の document](https://www.gatsbyjs.com/docs/how-to/routing/adding-markdown-pages/) を参考にプラグインを追加して構築したのですが、恥ずかしながら実際どの機能が何をどのようにして markdown ファイルから静的 HTML ファイルを生成しているのかについてはイマイチわかっていないままでした。
...
`gatsby-source-filesystem` と `gatsby-transformer-remark` がそれぞれ以下のような処理を以下の順番で行うことで実現されていました。

1. `gatsby-source-filesystem` が markdown ファイルを File node に変換

2. `gatsby-transformer-remark` が File node に含まれている markdown コンテンツから frontmatter を解析( markdown 部分と frontmatter を分離)
...
結論の内容を説明してみます。
内容は「処理部分」と「Gatsby Node API」という二つの観点で解説をしました。

ちなみに、なぜ「Gatsby Node API」を確認しているかというと、それが結論で書いた各処理がその順番で行われることの根拠になるからです。

各処理は最終的には plugin からなんらかの Gatsby Node API として export されています。そのため「どの Gatsby Node API が build プロセス中のどのタイミングで実行されるか」を確認しその順序で処理を並べると、それがすなわち処理順序となります。

Gatsby の build プロセスや Gatsby Node API については、私も今回初めて知ったことが多く学びになりました。実際に私が参考にした document を載せておきます。
...
- 処理部分
  ...
  - https://github.com/gatsbyjs/gatsby/blob/72f15717e9f166e338ba120591e8b4e1e8a92c16/packages/gatsby-transformer-remark/src/on-node-create.js#L32-L41
    ...
    - [gray-matter](https://github.com/jonschlinkert/gray-matter) を用いて frontmatter の解析( markdown 部分と frontmatter を分離)
...
markdown を HTML に変換する resolver
...
- 処理部分
  ...
  - markdown を HTML に変換する resolver が実装されています。ここが query が html field にアクセスする際に見にいく実体みたいです。この部分は後述の[resolver の正体](#resolverの正体)でさらに詳しく触れます。
- Gatsby Node API: `setFieldsOnGraphQLNodeType`
  ...
  - `setFieldsOnGraphQLNodeType` は Gatsby の build プロセスのうち GraphQL schema の作成中に実行される API です。API 実行結果として得られるオブジェクト配列を対象の Node の types に一つ一つ Filed として追加していきます。ですので今回で言えば「処理部分」で返されているオブジェクトが追加されていきます。

### 5. (page) Query において「html」field を指定することによって処理 4 で作成された resolver が実行され html が生成される。それが返り値として component に渡され描画される

ここは Gatsby の build プロセスのうち、1~4 の処理を全て終えブログ構築に必要な data が全てそろった後に実行されます。

以上が説明になります。

ここまでの内容が見えたあたりで、自分の中では問いに対してある程度の答えはつかめました。

ただ実際に markdown を html に変換する resolver が一体何をしているのかがまだはっきりとわかっていなかったです。なのでそれについても調べてみたので次章でまとめます。
...
remark 自体の理解にも結構苦しみました。
個人的には remark が所属している unified というプロジェクトの README がわかりやすかったです。参考にさせていただいたブログも合わせて載せておきます。
...
まともにソースコードを読んだ経験はまだまだすくなかったのですが、今回こうしてプラグインのコードを読めたことは自信につながりました。プラグインのコードは分量もそこまで多くなく、かつ周辺知識の習得はきちんとできるので、今の自分にはあっている気がしました。また他のパッケージのコードは読んでみようと思います。
